...
<template>
  <div class="bg-my-green rounded-xl shadow-md relative">
    <div v-if="authStore.getRole === 'ADMIN'" class="mt-2 flex justify-end">
      <button
        @click="handleDelete(props.details.id)"
    ...
    <div class="mb-2 p-4 flex flex-col items-center">
      <div
        class="image-container w-full h-[20vh] overflow-hidden relative mb-6"
      >
        <img
          v-if="details.photo"
      ...
      <div class="flex items-center mt-3 mb-3">
        <p class="text-my-white font-bold text-base">
          Category: {{ details.category }}
        </p>
      </div>
      <div class="flex items-center mb-3">
        <p class="text-my-white font-bold text-base">
          Found Date: {{ details.foundDate }}
        </p>
      </div>
      <RouterLink
        :to="'/found/' + details.id"
      ...
      <RouterLink
        v-if="authStore.getRole === 'USER'"
        :to="`/claim/${details.id}`"
        class="bg-blue-600 text-white font-bold hover:bg-blue-700 hover:text-white mt-2 px-4 py-2 rounded-lg text-center text-sm"
      >
        This is my item
      </RouterLink>
    </div>
